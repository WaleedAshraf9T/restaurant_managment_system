...
// Note: repository folder is `uploads` (lowercase). Use lowercase to avoid case-sensitive fs issues.
...
const pool = mysql.createPool({
  ...
  database: process.env.DB_NAME || 'r_restaurant',
...
// WebSocket server setup (port configurable via WS_PORT env)
// NOTE: Serverless environments (like Vercel) do not support running a persistent WebSocket server.
// We only start the WS server when running the backend as a standalone process (locally or on a persistent host).
...
const setupWebSocketServer = () => {
  wss = new WebSocket.Server({ port: parseInt(WS_PORT, 10) });
...
// Export the express app so it can be used in serverless environments (Vercel).
// Start the HTTP server and the WebSocket server only when running this file directly.
if (require.main === module) {
  // Start HTTP server (HTTP port configurable via PORT env)
  const HTTP_PORT = process.env.PORT || 3001;
  app.listen(HTTP_PORT, () => console.log(`Backend running on http://localhost:${HTTP_PORT}`));

  // Start WebSocket server only when running standalone.
  try {
    setupWebSocketServer();
    console.log(`WebSocket server running on ws://localhost:${WS_PORT}`);
  } catch (err) {
    console.warn('Failed to start WebSocket server:', err.message);
  }
} else {
  console.log('Backend loaded as a module (serverless mode). HTTP server and WebSocket server are NOT started.');
}
