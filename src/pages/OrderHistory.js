...
function OrderHistory() {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      setLoading(true);
      try {
        console.log('Fetching order history from:', `http://localhost:3001/api/tenants/${tenantId}/order-history`);
        const response = await axios.get(`http://localhost:3001/api/tenants/${tenantId}/order-history`, {
          headers: { Authorization: `Bearer ${token}` },
        });
