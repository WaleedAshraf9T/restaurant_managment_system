...
function Menu() {
  ...
  useEffect(() => {
    const fetchTenant = async () => {
      ...
      try {
        console.log('Fetching tenant from:', `http://localhost:3001/api/tenants/${tenantId}`);
    ...
    const fetchMenuItems = async () => {
      ...
      setLoading(true);
      try {
        console.log('Fetching menu items from:', `http://localhost:3001/api/tenants/${tenantId}/menu-items`);
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:3001/api/tenants/${tenantId}/menu-items`, itemData, {
        headers: { 
      ...
      if (response.data.success) {
        const updatedItems = await axios.get(`http://localhost:3001/api/tenants/${tenantId}/menu-items`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleUpdateItem = async () => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3001/api/tenants/${tenantId}/menu-items/${editItem.item_id}`, itemData, {
        headers: { 
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        },
  ...
  const handleDeleteItem = async (itemId) => {
    ...
    try {
      ...
      const response = await axios.delete(`http://localhost:3001/api/tenants/${tenantId}/menu-items/${itemId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleRestock = async (itemId) => {
    ...
    try {
      ...
      const response = await axios.patch(`http://localhost:3001/api/tenants/${tenantId}/menu-items/${itemId}/restock`, 
