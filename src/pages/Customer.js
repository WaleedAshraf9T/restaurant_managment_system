...
function Customer() {
  ...
  useEffect(() => {
    ...
    const fetchMenu = async () => {
      setLoading(true);
      try {
        console.log('Fetching menu from:', `http://localhost:3001/api/tenants/${tenantId}/public/menu`);
        const response = await axios.get(`http://localhost:3001/api/tenants/${tenantId}/public/menu`);
  ...
  useEffect(() => {
    if (orderStatus && !wsRef.current) {
      const refetchOrderStatus = async () => {
        try {
          console.log('Refetching order status from:', `http://localhost:3001/api/tenants/${tenantId}/orders/${orderStatus.order_id}`);
          const response = await axios.get(
            `http://localhost:3001/api/tenants/${tenantId}/orders/${orderStatus.order_id}?customerPhone=${encodeURIComponent(orderStatus.customer_phone)}`
          );
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:3001/api/tenants/${tenantId}/public/orders`,
        orderData
      );
