...
function SuperAdmin() {
  ...
  const fetchTenants = async () => {
    ...
    setLoading(true);
    try {
      console.log('Fetching tenants from:', 'http://localhost:3001/api/tenants');
      const response = await axios.get('http://localhost:3001/api/tenants', {
  ...
  const handleBlockTenant = async (tenantId, blocked) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3001/api/tenants/${tenantId}/block`, { blocked }, {
        headers: { Authorization: `Bearer ${token}` },
      });
