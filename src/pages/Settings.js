...
function Settings() {
  ...
  const handleUpdateSettings = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.put(`http://localhost:3001/api/tenants/${tenantId}`, formData, {
        headers: {
      ...
      if (response.data.success) {
        ...
        setError('');
        const tenantResponse = await axios.get(`http://localhost:3001/api/tenants/${tenantId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setTenant({
